      ...
      <div class="th_property_report-cont" ref="cont">
        <div >
          <mt-cell
            ...
            <span class="color999">{{newSelectTextList[0] || '请选择报备楼盘'}}</span>
          </mt-cell>
          <mt-cell
          ...
          <div class="tips">
            报备提示：
            {{newSelectPropertyRemarkList[0] || '无'}}</div>
        </div>
        <div v-if="submitData.secondId">
          ...
          <div class="tips">报备提示：{{newSelectPropertyRemarkList[1]|| '无'}}</div>
        </div>
        <div v-if="submitData.thirdId">
          ...
          <div class="tips">报备提示：{{newSelectPropertyRemarkList[2]|| '无'}}</div>
        </div>
        <ul class="inputBox">
          <li>
...
export default {
  ...
  created () {
    // 添加默认的楼盘
    // this.newSelectList.push(this.$route.params.id)
    // this.newSelectTextList.push(this.$route.query.buildingGroupName)
    // this.newSelectPropertyRemarkList.push(this.$route.query.reportRemark)
    try {
      // 报备数据-看房服务
    ...
    try {
      // 楼盘选择-楼盘id
      let newSelectList = sessionStorage.getItem('newSelectPropertyList')
      if (newSelectList) {
        this.newSelectList = JSON.parse(newSelectList)
        this.$set(this.submitData, 'firstId', this.newSelectList[0])
        this.$set(this.submitData, 'secondId', this.newSelectList[1])
        this.$set(this.submitData, 'thirdId', this.newSelectList[2])
      }
    } catch (error) {}
    try {
      // 楼盘选择-楼盘name
      let newSelectTextList = sessionStorage.getItem('newSelectPropertyTextList')
      newSelectTextList && (this.newSelectTextList = JSON.parse(newSelectTextList))
    } catch (error) {}
    try {
      // 楼盘选择-楼盘报备标注
      let newSelectPropertyRemarkList = sessionStorage.getItem('newSelectPropertyRemarkList')
      newSelectPropertyRemarkList && (this.newSelectPropertyRemarkList = JSON.parse(newSelectPropertyRemarkList))
    } catch (error) {}
  ...
  methods: {
    ...
    selectProperty () {
      if (!this.$route.query.back) {
        this.setSessionStorage()
        this.$router.push({path: '/newSelectProperty/' + this.$route.params.id,
          query: {
            buildingGroupName: this.$route.query.buildingGroupName,
    ...
    /**
       ...
       */
    setSessionStorage () {
      sessionStorage.setItem('newSelectPropertyList', JSON.stringify(this.newSelectList))
      sessionStorage.setItem('newSelectPropertyTextList', JSON.stringify(this.newSelectTextList))
      sessionStorage.setItem('newSelectPropertyRemarkList', JSON.stringify(this.newSelectPropertyRemarkList))
